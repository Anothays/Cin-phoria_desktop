...
export default function Seat({
  ...
  const { setSelectedSeats } = useSeatMapContext();
  ...
  useEffect(() => {
    setSelectedSeats!((prev) => {
      if (isSelected) {
        ...
        return prev.filter((el) => el.id !== seat.id);
      }
    });
  }, [isSelected]);
    ...
    <svg
      onClick={() => {
        if (!isNotSelectable) {
          ...
          alert(
            "Cette place n'est plus disponible, veuillez-choisir une autre place"
          );
        }
      }}
